...
class UartCom():
    ...
    def __init__(self, ui, eventThread, dbInsertManagerThread):
        ...
        ui.fan1_f.clicked.connect(lambda x: self.controlFanPower('1'))
        ui.fan2_f.clicked.connect(lambda x: self.controlFanPower('2'))
        ui.uv_f.clicked.connect(self.controlUVPower)
        for i, ledButton in enumerate(eventThread.ledButton_f_list):
            ledButton.clicked.connect(partial(self.controlLEDPower,str(i+1)))
        self.uart = None
    ...
    def controlLEDPower(self, ledID):
        if SETTINGS.led['led'+ledID] == 'off':
...
class RcvParser(PyQt5.QtCore.QObject):
    ...
    def rcvLEDpower(self, command):
        ...
        try :
            ...
            SETTINGS.led['led'+ledID] ='off' if command[4:7] =='000' and command[8:11] == '000' \
                                    and command[12:15]=='000' and command[16:] == '000' else 'on'
            self.updateActuatorSignal.emit('led'+ledID, SETTINGS.led['led'+ledID])
        except Exception as e:
            print(str(e))
...
class EventThread(PyQt5.QtCore.QThread):
    ...
    def __init__(self, ui):
        ...
        self.mainButton_list = [ui.main, ui.elec, ui.settings]
        ...
        self.ledButton_f_list = [ui.led1_f, ui.led2_f, ui.led3_f, ui.led4_f, ui.led5_f, ui.led6_f, ui.led7_f, ui.led8_f]  
        ...
        ui.elec_day.clicked.connect(lambda x:self.updateELECPlot(True))
        ui.elec_week.clicked.connect(lambda x:self.updateELECPlot(False))
        ui.elec_month.clicked.connect(lambda x:self.updateELECPlot(False))

        ui.elec.clicked.connect(self.updateELEC)
        ui.elec.clicked.connect(partial(self.classifying, ui.elec_day, update = 'elec'))
        ui.settings.clicked.connect(lambda x : self.updateSettings(ui, sensor='pushButton'))
        
        ui.serv_add_save.clicked.connect(lambda x:self.updateSettings(ui, sensor = 'serv_add'))
        ui.serv_add_save.clicked.connect(lambda x: self.alert('프로그램 종료 후 재실행 시 적용됩니다.'))
        ui.serv_save.clicked.connect(lambda x:self.updateSettings(ui, sensor = 'serv'))
        ui.serv_save.clicked.connect(lambda x:self.alert('프로그램 종료 후 재실행 시 적용됩니다.'))
        
        ui.sens_save.clicked.connect(lambda x: self.updateSettings(ui, sensor = 'sens'))
        ui.sens_save.clicked.connect(lambda x: self.alert('프로그램 종료 후 재실행 시 적용됩니다.\n센서 측정 최소 주기는 10초입니다.'))

        ui.save_tds.clicked.connect(lambda x : self.alert('TDS 최소 설정값은 TDS 최대 설정값보다 클 수 없습니다.'if ui.tds_l.value() > ui.tds_h.value() else ''))
        ui.save_tds.clicked.connect(lambda x: self.updateSettings(ui,sensor='save_tds'))
        ui.save_ph.clicked.connect(lambda x: self.alert('PH 최소 설정값은 PH 최대 설정값보다 클 수 없습니다.' if ui.ph_l.value() > ui.ph_h.value() else ''))
        ui.save_ph.clicked.connect(lambda x: self.updateSettings(ui, sensor='save_ph'))
    ...
    def updateActuator(self, button, state):
        classify = {'on': True, 'off':False}
        if button =='uv':
            ui.uv_b.setChecked(classify[state])
        elif button =='fan1':
            ui.fan1_b.setChecked(classify[state])
        elif button =='fan2':
            ui.fan2_b.setChecked(classify[state])
        elif button[0:3] =='led':
            self.ledButton_b_list[int(button[-1])-1].setChecked(classify[state])
        ...
        if INIT:
            with open ('config.json', 'r') as jsonFile:
                ...
                for ledID in range(1,9):
                    SETTINGS.led['led'+str(ledID)] = settingsData['led']['led'+str(ledID)]
                ...
                for index in range(0,8):
                    if settingsData['led']['led'+str(index+1)] == 'on':
                        self.ledButton_b_list[index].setChecked(True)
                    else:
                        self.ledButton_b_list[index].setChecked(False)
        if sensor == 'pushButton':
            for i,lineEdit in enumerate([ui.ip1, ui.ip2, ui.ip3, ui.ip4]):
                lineEdit.setText(SETTINGS.serv_add.split('.')[i])
            print(SETTINGS.serv_port)
...
def stopall(eventThread, valueUpdateThread, timeUpdateThread, uartCom, dbInsertManagerThread, dbFetchManagerThread):
    ...
    with open ('config.json', 'r') as jsonFile:
        ...
        for index in range(1,9):
            settingsData['led']['led'+str(index)] = SETTINGS.led['led'+str(index)]
        ...
        classify =  {'sens_freq':SETTINGS.sens_freq, 'sens_unit':SETTINGS.sens_unit,
